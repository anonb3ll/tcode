...
- UA Control Center (first slice, task #635): desktop **View → Open Unified Agent Control Center** loads the Mac hub `/ui` in the existing preview pane (default `http://100.111.5.64:8765/ui`, override with `T3CODE_UA_CONTROL_CENTER_URL`). Auth is hub `ua_ui` cookie via BrowserSession — not MCP bearer. Origin/CORS follow-ups are task #636 if embed is blocked.
