...
export const AntigravitySettings = makeProviderSettingsSchema(
  {
    ...
    model: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Default model",
        description: "Default model for Antigravity sessions (e.g. gemini-3.7-flash-high).",
        providerSettingsForm: { placeholder: "gemini-3.7-flash-high", clearWhenEmpty: "omit" },
      }),
    ),
    effort: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        ...
        providerSettingsForm: { placeholder: "low", clearWhenEmpty: "omit" },
      }),
    ),
    customModels: Schema.Array(Schema.String).pipe(
      Schema.withDecodingDefault(Effect.succeed([])),
  ...
  {
    order: ["binaryPath", "model", "effort", "launchArgs"],
  },
);
